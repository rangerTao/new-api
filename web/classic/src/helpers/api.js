...
import { appBasePath, getAppUrl } from './basePath';

const apiBaseURL = import.meta.env.VITE_REACT_APP_SERVER_URL || appBasePath;
...
export async function onDiscordOAuthClicked(client_id, options = {}) {
  ...
  if (!state) return;
  const redirect_uri = getAppUrl('/oauth/discord');
...
export async function onOIDCClicked(
  ...
  url.searchParams.set('redirect_uri', getAppUrl('/oauth/oidc'));
...
export async function onCustomOAuthClicked(provider, options = {}) {
  ...
  try {
    const redirect_uri = getAppUrl(`/oauth/${provider.slug}`);
