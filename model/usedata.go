...
func SaveQuotaDataCache() {
	...
	for _, quotaData := range CacheQuotaData {
		...
		if quotaDataDB.Id > 0 {
			...
			increaseQuotaData(quotaData.UserID, quotaData.Username, quotaData.ModelName, quotaData.Count, quotaData.Quota, quotaData.CreatedAt, quotaData.TokenUsed, quotaData.PromptTokens, quotaData.CompletionTokens)
		} else {
...
func increaseQuotaData(userId int, username string, modelName string, count int, quota int, createdAt int64, tokenUsed int, promptTokens int, completionTokens int) {
...
func GetQuotaDataGroupByUser(startTime int64, endTime int64, modelName string) (quotaData []*QuotaData, err error) {
	...
	tx := DB.Table("quota_data").
		Select("username, created_at, sum(count) as count, sum(quota) as quota, sum(token_used) as token_used, sum(prompt_tokens) as prompt_tokens, sum(completion_tokens) as completion_tokens").
...
func GetAllQuotaDates(startTime int64, endTime int64, username string, modelName string) (quotaData []*QuotaData, err error) {
	...
	var quotaDatas []*QuotaData
	tx := DB.Table("quota_data").
		Select("model_name, sum(count) as count, sum(quota) as quota, sum(token_used) as token_used, sum(prompt_tokens) as prompt_tokens, sum(completion_tokens) as completion_tokens, created_at").
		Where("created_at >= ? and created_at <= ?", startTime, endTime)
	if modelName != "" {
		tx = tx.Where("model_name = ?", modelName)
	}
